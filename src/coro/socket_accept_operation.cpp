...
bool coro::net::socket_accept_operation_impl::try_start(coro::detail::io_operation_base &operation) noexcept {
    return operation.m_io_queue.transaction(&operation.m_message)
        .accept(m_listen_sock.native_handle(), &m_address_buf, m_buf_len)
        .commit();
}
...
void coro::net::socket_accept_operation_impl::get_result(coro::detail::io_operation_base &operation) {
    ...
    if (getpeername(fd, &m_address_buf, &m_buf_len) < 0) {
        throw std::system_error{errno, std::generic_category()};
    }
    m_accept_sock.m_remote_endpoint = detail::sockaddr_to_ip_endpoint(std::ref(m_address_buf));
    m_buf_len = sizeof(m_address_buf);
    if (getsockname(fd, &m_address_buf, &m_buf_len) < 0) {
        throw std::system_error{errno, std::generic_category()};
    }
    m_accept_sock.m_local_endpoint = detail::sockaddr_to_ip_endpoint(std::ref(m_address_buf));
}
