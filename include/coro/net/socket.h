...
#include "../detail/macos_kqueue_queue.h"
...
public:
    ...
    void bind(const ip_endpoint& local_endpoint) {
        ...
        SOCKADDR* boundSockaddr = reinterpret_cast<SOCKADDR*>(&bound_sockaddr_storage);
        socklen_t boundSockaddrLen = sizeof(bound_sockaddr_storage);
        result = getsockname(m_handle, boundSockaddr, &boundSockaddrLen);
        if (result == 0) {
            m_local_endpoint = detail::sockaddr_to_ip_endpoint(*boundSockaddr);
        }
        else {
